...
import { ref, onValue, update, runTransaction, push } from "firebase/database"
...
export const VoterProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
                                                ...
                                                (snapshot) => {
                                                        try {
                                                                ...
                                                                if (data && Array.isArray(data)) {
                                                                        ...
                                                                        const votersData = data.map((value: any, index: number) => ({
                                                                                ...value,
                                                                                firebaseKey: index.toString(), // Use index as key since it's an array
                                                                                village,
                                                                                division,
                                                                                vibhag,
                                                                        }))
                                                                        ...
                                                                        // Fallback: handle object structure if needed
        ...
        const updateVoterStatus = useCallback(
                async (voter: Voter, field: string, value: any) => {
                        ...
                        try {
                                // For array structure, we need to update the entire array or find a better approach
                                // Since Firebase Realtime Database doesn't support direct array index updates well,
                                // we might need to restructure the data or use a different approach

                                // For now, let's update the entire array item
                                const votersRef = ref(
                                        database,
                                        `villages/${voter.village}/${voter.division}/${voter.vibhag}/मतदार_यादी/${voter.firebaseKey}`,
                                )

                                await update(votersRef, { [field]: value })
                                ...
                                try {
                                        ...
                                        await update(activityRef, {
                                                ...
                                                voterName: voter["मतदाराचे_पूर्ण_नांव"] || voter["नाव"] || null,
