...
const RegisterScreen = ({ navigation }) => {
        ...
        const handleRegister = async () => {
                ...
                try {
                        ...
                        console.log(`[Registration] Generated OTP for ${mobileNumber}: ${otp}`);

                        // Send OTP via SMS
                        console.log(`[Registration] Attempting to send OTP to ${mobileNumber}`);
                        const { success, message } = await sendOTP(mobileNumber, otp);

                        if (!success) {
                                console.error(`[Registration] OTP sending failed: ${message}`);
                                Alert.alert(
                                        "त्रुटी",
                                        `OTP पाठविण्यात अडचण आली: ${message || "अज्ञात त्रुटी"}\nकृपया पुन्हा प्रयत्न करा.`,
                                        [{ text: "ठीक आहे" }]
                                );
                                return;
                        }

                        console.log(`[Registration] OTP sent successfully, creating user account`);
                        const email = `${mobileNumber}@gmail.com`;

                        // Create user with Firebase Auth using OTP as password
                        const userCredential = await createUserWithEmailAndPassword(auth, email, otp);
                        const user = userCredential.user;
                        console.log(`[Registration] User created with UID: ${user.uid}`);
                        ...
                        Alert.alert("यशस्वी!", "तुमचे खाते यशस्वीरित्या तयार झाले आहे. OTP तुमच्या मोबाईल नंबरवर पाठवला आहे.", [
